...
const createInitialOrder = () => {
  ...
  let backendUrl = "https://guarded-cove-46425.herokuapp.com/orders/startOrder"
...
const Home = () => {
  ...
  return (
    <Layout>
      ...
      <h3 className={styles.title}>You are at <span className={styles.table}>Table 53</span>.</h3>
      <div className={styles.options}>
        <Link to="/menu">View Menu</Link>
        <Link to="/" onClick={callHost}>
          Call Waiter/Waitress
        </Link>
        <Link to="/order-progress">Check on Order Progress</Link>
        <div className={styles.paymentOptions}>
          <Link to="/" onClick={callCheck}>
            Ask for the Check
          </Link>
